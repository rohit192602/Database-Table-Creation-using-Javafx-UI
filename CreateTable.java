...
public class CreateTable extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Label titleLabel = new Label("Database Table Creation Using javafx");
        ...
        grid.add(titleLabel, 1, 0, 2, 1);

        TextField tableNameField = new TextField();
        TextField columnNameField = new TextField();
        TextField dataField = new TextField(); // For data entry
        ...
        Button addDataButton = new Button("Add Data"); // Button to add data
        ...
        grid.add(new Label("Enter Data:"), 0, 5); // Label for data entry
        grid.add(dataField, 1, 5); // TextField for data entry
        grid.add(addDataButton, 2, 5); // Button for adding data
        grid.add(logArea, 0, 6, 3, 1);

        // Remove the previous section that displays tableView and tableContentView
        // grid.add(tableView, 0, 6, 3, 1); // Removed this line
        grid.add(tableContentView, 0, 7, 3, 1); // Place TableContentView only
        ...
        addDataButton.setOnAction(e -> {
            ...
            if (!tableName.isEmpty() && !data.isEmpty()) {
                ...
                try (Statement statement = connection.createStatement()) {
                    statement.execute(sql);
                    logArea.appendText("Data '" + data + "' added to table '" + tableName + "'.\n");
                    log("Data '" + data + "' added to table '" + tableName + "'.");
                    dataField.clear();
                    // Refresh table content view to reflect changes
                    showTableButton.fire();
                } catch (SQLException ex) {
                    logArea.appendText("Error adding data: " + ex.getMessage() + "\n");
                    log("Error adding data: " + ex.getMessage());
                }
            }
        });
